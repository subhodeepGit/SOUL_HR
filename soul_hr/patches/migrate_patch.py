...
#   bench execute soul_hr.patches.migrate_patch.get_custom_role_permission
...
#   bench execute soul_hr.patches.migrate_patch.set_custom_role_permission
def set_custom_role_permission():
    with open(frappe.get_app_path("soul_hr","fixtures","custom_docperm.json")) as f:
...
#   bench execute soul_hr.patches.migrate_patch.add_roles
def add_roles():
    with open(frappe.get_app_path("soul_hr","fixtures","role.json")) as f:
        for d in json.load(f):
...
def add_module_profile():
    with open(frappe.get_app_path("soul_hr","fixtures","module_profile.json")) as f:
        for d in json.load(f):
